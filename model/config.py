...
All secrets (Ollama API key) and model choices are read from environment
variables so nothing is hardcoded. Copy `.env.example` to `.env` and
fill in your own OLLAMA_API_KEY before running the backend.
"""
...
AUTO_DETECT_SHORTLIST = ["hi", "bn", "ta", "te", "mr"]
...
class Settings:
    ...
    translation_indic_en_model_id: str = field(default_factory=lambda: _env(
        "TRANSLATION_INDIC_EN_MODEL_ID", "ai4bharat/indictrans2-indic-en-dist-200M"
    ))
    translation_en_indic_model_id: str = field(default_factory=lambda: _env(
        "TRANSLATION_EN_INDIC_MODEL_ID", "ai4bharat/indictrans2-en-indic-dist-200M"
    ))

    # -- LLM: Gemma 4 via Ollama Cloud (English question -> English answer) ---
    ...
    ollama_model: str = field(default_factory=lambda: _env("OLLAMA_MODEL", "gemma4:cloud"))

    # -- Text-to-speech (English/native answer -> spoken audio) ----------------
    tts_model_id: str = field(default_factory=lambda: _env(
